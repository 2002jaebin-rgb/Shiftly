  ...
  availabilitiesWeekly: {
    ...
    create: async (storeId, store_week_id, weekday, start_time, end_time) => {
      ...
      const { data, error } = await supabase
        .from('availabilities')
        .insert([
          { store_id: storeId, store_week_id, user_id: uid, weekday, start_time, end_time },
        ])
        .select()
        .single()
      return { data, error }
  ...
  shiftNeeds: {
    ...
    create: async (
      ...
      const { data, error } = await supabase
        .from('shift_needs')
        .insert([
          { store_id: storeId, date, start_time: startTime, end_time: endTime, required_staff: requiredStaff, due_date: dueDate },
        ])
        .select()
        .single()
      return { data, error }
  ...
  availabilities: {
    ...
    create: async (storeId, needId, date, startTime, endTime) => {
      ...
      const { data, error } = await supabase
        .from('availabilities')
        .insert([
          { store_id: storeId, need_id: needId, user_id: userId, date, start_time: startTime, end_time: endTime },
        ])
        .select()
        .single()
      return { data, error }
